...
class OurStyleGAN2Loss(Loss):
    ...
    def run_D(self, img, c, sync):
        ...
        with misc.ddp_sync(self.D, sync):
            logits, img_logits, temp_imgs, c_logits, c_img_logits  = self.D(img, c)
        return logits, img_logits, temp_imgs, c_logits, c_img_logits
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain):
        # print('real c', real_c) # label [1,0,0,0,0,0,0]
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                gen_logits, gen_img_logits, _ , gen_c_logits, gen_c_img_logits = self.run_D(gen_img, gen_c, sync=False)
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
                loss_Gmain = torch.nn.functional.softplus(-gen_logits) # -log(sigmoid(gen_logits))
                loss_Gimg_main = torch.nn.functional.softplus(-gen_img_logits)
                # print('gen_c_logits',gen_c_logits) #gen_c_logits tensor([[8.5800e-04, 4.5125e-03, 2.5463e-02, 5.5540e-01, 2.9698e-01, 1.1408e-01, 2.7009e-03]
                # print('gen_c',gen_c) #gen_c tensor([[1., 0., 0., 0., 0., 0., 0.]
                # g_loss = 0.5 * (loss_Gmain + auxiliary_loss(gen_c_logits, gen_c.long()))
                # training_stats.report('g_loss', g_loss)
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                ...
                # print(gen_c_logits)
                # print(gen_c_logits.shape)
                # d_fake_loss = (gen_logits + auxiliary_loss(gen_c_logits, gen_c)) / 2
                # training_stats.report('d_fake_loss', d_fake_loss)
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                real_logits, real_img_logits, temp_imgs, real_c_logits, real_c_img_logits = self.run_D(real_img_tmp, real_c, sync=sync)
                ...
                if do_Dmain:
                    ...
                    # d_real_loss = ((loss_Dgen + loss_Dreal) + auxiliary_loss(real_c_logits, real_c)) / 2
                    # print(real_c)
                    # print(real_c.shape)
                    # print(real_c_logits)
                    # print(real_c_logits.shape)
                    # training_stats.report('d_real_loss', d_real_loss)
                    training_stats.report('Loss/D/loss', loss_Dgen + loss_Dreal)
                    training_stats.report('Loss/D/Iloss', loss_Dimg_gen + loss_Dimg_real)
                    # d_loss = (d_real_loss + d_fake_loss) / 2
                    # training_stats.report('d_loss', d_loss)
                    # pred = np.concatenate([real_c_logits.data.cpu().numpy(), gen_c_logits.data.cpu().numpy()], axis=0)
                    # gt = np.concatenate([real_c.data.cpu().numpy(), gen_c.data.cpu().numpy()], axis=0)
                    # d_acc = np.mean(np.argmax(pred, axis=1) == gt)
                    # print('pred',pred,'gt', gt,'d_acc', d_acc)
                loss_Dr1 = 0
                loss_Dimg_r1 = 0
                if do_Dr1:
