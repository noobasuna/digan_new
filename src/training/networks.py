...
class DiscriminatorEpilogue(torch.nn.Module):
    def __init__(self,
        ...
        # self.fcc = nn.Sequential(nn.Linear(in_channels * (resolution ** 2), 7), nn.Softmax())
        self.fcc = nn.Sequential(nn.Linear(in_channels, 7))
        self.out = FullyConnectedLayer(in_channels, 1 if cmap_dim == 0 else cmap_dim)
        self.out_c = FullyConnectedLayer(in_channels, 7 if cmap_dim == 0 else cmap_dim)
...
class ImageDiscriminator(torch.nn.Module):
    ...
    def forward(self, img, c, **block_kwargs):
        ...
        x, y = self.b4(x, img, cmap)
        # y = self.b4(x, img, cmap)
        return x, y
...
class OurDiscriminator(torch.nn.Module):
    ...
    def forward(self, vid, c, **block_kwargs):
        ...
        x_vid, y_vid = self.videoD(vid, c, **block_kwargs)
        x_img, y_img = self.imageD(img, cc, **block_kwargs)

        return x_vid, x_img, img, y_vid, y_img
